...
const uint DHT_SENSOR_PIN = 28;

int previousButtonValue = 1;

void sensor_task(void *pvParameters) { // TODO: 
    while (true) {
...
void btn_cb(void *pvParameters) { // TODO:
    while (true) {
...
int main() {
    ...
    adc_init();
    adc_gpio_init(DHT_SENSOR_PIN);
    adc_select_input(2);

    xTaskCreate(sensor_task, "LED_Task", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
    xTaskCreate(btn_cb, "ButtonTask", configMINIMAL_STACK_SIZE, NULL, 10, NULL);
