...
def calculate_new_position(new_pos, new_angle, direction):
    ...
    new_pos[0] += math.cos(new_angle) * speed * direction[0]
    new_pos[1] += math.sin(new_angle) * speed * direction[1]
    return new_pos
...
def hit_wall(cordinate, boundaries, angle, lst, direction):
    ...
    while not found_direction:
        ...
        # position
        # makes sure that cordinate[0] - current_list[0] != 0 as it will
        # result in a ZeroDivisionError
        ...
        while not found_direction:
            ...
            # if new_pos is in boundaries it breaks and tries again
            # with new cordinate
            if [int(new_pos[0]), int(new_pos[1])] in boundaries:
                times_tried += 1
                break
            # if the new position is uncut it returns the angle and direction
...
def change_position(speed, current_pos, angle, direction, boundaries, lst,
    ...
    length = 0
    line = [current_pos[0], current_pos[1]]
    pos_list = []
    boundary = False
    time = 0
    while not boundary:
        ...
        if ([math.floor(line[0]), math.floor(line[1])] in boundaries):
            boundary = True
        time += 1
    # deletes the latest cordinate due to it being in the boundary
    pos_list.pop(-1)
    # takes one step back
...
def get_back(cordinate, boundaries, time_gone, movement_history, multiplier):
    ...
    while not found_goal:
        ...
        while not found_direction:
            # Tests whether the position is accessible without running into a
            # wall
            new_pos = calculate_new_position(new_pos, new_angle, direction)
            if [int(new_pos[0]), int(new_pos[1])] in boundaries:
                times_tried += 1
            ...
            if [int(new_pos[0]), int(new_pos[1])] == start_cordinate:
                ...
                time_gone += ((math.sqrt(distance))/multiplier)/speed
                ...
                # breaks if it has reached the start point else it tries again
                if (times_tried == 0):
                    break
                else:
...
def main(path, multiplier, minutes, max_percentage, condition, delta_t, show):
    ...
    while available_conditions[condition]:
        ...
        actual_time += (time_gone*delta_t)/multiplier
        # Turns.
    ...
    if show:
        ...
        pcm(lst, actual_time, round(percentage, 2), (start_un_cut - un_cut),
            start_un_cut)
