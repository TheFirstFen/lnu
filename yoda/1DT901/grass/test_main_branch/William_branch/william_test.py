...
def change_position(speed: float, direction: list,
                    angle: float, current_pos: list) -> list:
...
def main(path, multiplier, minutes, show):
    ...
    precentage = 100 - ((un_cut / start_un_cut) * 100)
    ...
    if show:
        ...
        # pixel_map(const_map)
        pcm(lst, actual_time, precentage,
            (start_un_cut - un_cut), start_un_cut)

main("ground_maps/simple.csv", 5, 0, False)